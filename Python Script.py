import serial
import paho.mqtt.client as mqtt         # to initialize the MQTT client
...
# MQTT Configuration
MQTT_BROKER = "broker.mqtt.cool"  # Replace with your broker hostname/ip address (some are use - "test.mosquitto.org")
MQTT_PORT = 1883
...
# Regular expression to match data format [\d - integer] 
data_regex = r"Received Data from Slave(\d+): Temp = ([\d.]+)°C, Humidity = ([\d.]+)%, Alert = (.*)"

try:
    while True:
        if ser.in_waiting > 0:
            line = ser.readline().decode('utf-8').strip()   # when data is received, it reads a line, decodes it, and removes whitespace.
