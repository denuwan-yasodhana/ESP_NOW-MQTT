...
#include <WiFi.h>
#include <DHT.h>
#include <esp_sleep.h>  // Provides deep sleep functionality to reduce power consumption

#define DHTPIN 4  // Data pin connected to DHT11 sensor
...
// Broadcast address for ESP-Now [MAC address of the recipient (the ESP-Now receiver). This address must match the target device]
uint8_t broadcastAddress[] = {0x08, 0xB6, 0x1F, 0x28, 0x77, 0xF0};

// User-configurable settings
const unsigned long communicationInterval = 3500;  // Communicate every 10 seconds [Defines the time(ms) for which the device sleeps before waking up to collect and send data]
...
void setup() 
{
  ...
  WiFi.mode(WIFI_STA);  // Station mode required for ESP-Now(to allow communication)
  ...
  // Set up peer-to-peer communication and add the recipient (broadcast) peer
...
void enterDeepSleep()  // It minimizes power usage while the device is idle.
{
